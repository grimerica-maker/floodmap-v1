...
export default function HomePage() {
  ...
  useEffect(() => {
    if (!isMapReady()) return;
    syncFloodScenario();
  }, [seaLevel]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 14, color: "#666", marginBottom: 24 }}>
          Flood-only stabilization build
        </div>
        ...
        <div style={{ display: "grid", gap: 10, marginBottom: 24 }}>
          ...
          >
            <div>Impact</div>
            <div style={{ fontSize: 13, opacity: 0.9, marginTop: 4 }}>
              Disabled until flood is stable
            </div>
          </button>
        </div>
      ...
      >
        ...
        <div>Impact: temporarily disabled</div>
