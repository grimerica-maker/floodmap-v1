...
const FRONTEND_BUILD_LABEL = "v42";
...
export default function HomePage() {
  ...
  const formatInputTextFromMeters = (meters, unit = unitMode) => {
    return unit === "ft"
      ? formatNumericText(metersToFeet(meters), 2)
      : formatNumericText(meters, 2);
  };
  ...
  const clearImpactScenarioState = () => {
    clearImpactPreview();
  ...
  const getImpactPreviewRadiiKm = (diameterM) => {
    ...
    return {
      ...
      blast: Math.max(1, d * 0.006),
      thermal: Math.max(2, d * 0.012),
    };
  };
  ...
  const drawOceanImpactFromResult = (lng, lat, result) => {
    ...
    try {
      ...
      if (DEBUG_FLOOD) {
        console.log("DRAW OCEAN RESULT OK", { lng, lat, runId: result.run_id });
      }
  ...
  const attachImpactFloodLayerNow = (runId) => {
    const map = mapRef.current;
    if (!map || !runId || !map.isStyleLoaded()) return false;
  ...
  const addImpactFloodLayer = (runId) => {
    ...
    pendingImpactFloodRunIdRef.current = runId;
    ...
    if (DEBUG_FLOOD) {
      console.warn("IMPACT FLOOD LAYER DEFERRED UNTIL STYLE LOAD", { runId });
    }
    return false;
  };
  ...
  const addFloodLayer = (level) => {
    const map = mapRef.current;
    if (!map || !map.isStyleLoaded() || !floodAllowedInCurrentView()) return false;
  ...
  const runImpact = async () => {
    ...
    try {
      ...
      if (data.is_ocean_impact === true && Number(data.tsunami_radius_m ?? 0) > 0) {
        ...
        const floodOk = data.run_id ? addImpactFloodLayer(data.run_id) : false;

        if (DEBUG_FLOOD) {
          console.log("OCEAN IMPACT APPLY", {
            oceanOk,
            floodOk,
            runId: data.run_id,
          });
        ...
        setStatus(
          floodOk || pendingImpactFloodRunIdRef.current
  ...
  useEffect(() => {
    ...
    const handleIdle = () => {
      if (
        pendingImpactFloodRunIdRef.current &&
        mapRef.current &&
        mapRef.current.isStyleLoaded()
      ) {
        attachImpactFloodLayerNow(pendingImpactFloodRunIdRef.current);
      }
    };
  ...
  useEffect(() => {
    ...
    if (impactResult.is_ocean_impact === true) {
      ...
      if (impactResult.run_id) addImpactFloodLayer(impactResult.run_id);
      return;
    }
        ...
        impactPointRef.current
          ? impactLoading
            ...
            ? impactResult.is_ocean_impact
              ? impactFloodRunId || pendingImpactFloodRunIdRef.current
                ? "Ocean impact simulation complete"
                : "Ocean impact result returned, flood layer failed to attach"
    ...
    >
      ...
      >
        ...
        <div
          style={{
            ...
            marginBottom: 12,
            color:
              seaLevel > 0 ? "#0f62fe" : seaLevel < 0 ? "#b45309" : "#111827",
          }}
        >
        ...
        <input
          ...
          onBlur={() => {
            ...
            if (committed !== null) setInputLevel(committed);
          }}
          onKeyDown={(e) => {
      ...
      >
        ...
        <div style={{ color: "#facc15", fontWeight: 700 }}>
          Frontend build: {FRONTEND_BUILD_LABEL}
        </div>
        ...
        {impactResult && (
          <>
            ...
            <div>
              Energy: {Number(impactResult.energy_mt_tnt ?? impactResult.energy_mt ?? 0).toFixed(2)} Mt
            </div>

            <div>
              Crater Diameter: {Math.round(Number(impactResult.crater_diameter_m ?? 0)).toLocaleString()} m
            </div>

            <div>
              Blast Radius: {Math.round(Number(impactResult.blast_radius_m ?? 0)).toLocaleString()} m
            </div>

            <div>
              Thermal Radius: {Math.round(Number(impactResult.thermal_radius_m ?? 0)).toLocaleString()} m
            </div>
            ...
            {impactResult.is_ocean_impact === true &&
              Number(impactResult.tsunami_radius_m ?? 0) > 0 && (
                <>
                  ...
                  <div>
                    Wave Height: {Math.round(Number(impactResult.wave_height_m ?? 0)).toLocaleString()} m
                  </div>
                </>
              )}
        ...
        <div>
          {waterDifference !== null
            ? waterDifference >= 0
              ...
              : unitMode === "ft"
              ? `Underwater by ${Math.round(metersToFeet(Math.abs(waterDifference)))} ft`
