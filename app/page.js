      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button className="share-btn" onClick={() => {
          const msg = impactResult
            ? `💥 I just simulated a ${Number(impactResult.diameter_m ?? 0).toLocaleString()}m asteroid impact on Disaster Map! ${Math.round(Number(impactResult.estimated_deaths ?? 0)).toLocaleString()} estimated deaths. Try it: [https://www.disastermap.ca`]https://www.disastermap.ca`
            : nukeResult
            ? `☢️ I just detonated a ${nukeResult.yield_kt >= 1000 ? (nukeResult.yield_kt/1000).toFixed(1)+"Mt" : nukeResult.yield_kt+"kt"} nuke on Disaster Map! ${Math.round(Number(nukeResult.estimated_deaths ?? 0)).toLocaleString()} estimated deaths. Try it: [https://www.disastermap.ca`]https://www.disastermap.ca`
            : (scenarioMode === "tsunami" && tsunamiActive)
            ? `🌊 Just triggered the ${TSUNAMI_SOURCES[tsunamiSource].name} mega-tsunami on Disaster Map!${tsunamiResult ? " Est. " + tsunamiResult.total_deaths.toLocaleString() + " deaths." : ""} Try it: [https://www.disastermap.ca`]https://www.disastermap.ca`
            : (scenarioMode === "yellowstone" && yellowstoneActive)
            ? `🌋 Just simulated the Yellowstone ${YELLOWSTONE_PRESETS[yellowstonePreset].name} supervolcano on Disaster Map!${yellowstoneResult ? " Est. " + yellowstoneResult.total_deaths.toLocaleString() + " deaths." : ""} Ash covers most of North America. Try it: [https://www.disastermap.ca`]https://www.disastermap.ca`
            : `🌊 I just flooded the world ${seaLevel > 0 ? "+" : ""}${Math.round(seaLevel)}m on Disaster Map!${floodDisplaced ? " " + floodDisplaced.toLocaleString() + " people displaced." : ""} Try it: [https://www.disastermap.ca`]https://www.disastermap.ca`;
